...
-- INSERT 하고자 하는 컬림이 모든 컬림인 경우 컬럼명 생략 가능.
...
ROLLBACK; -- COMMIT 되지 않은 걸 없던 일로 만듦 COMMIT 했으면 장채현 안 없어짐
...
-- DEPARTMENT2 테이블에서 DEPT_ID 가 'D9'인 행의 DEPT_TITLE 을
-- '전략기획팀'으로 수정
...
SELECT * FROM DEPARTMENT2;
-- 모든 행에 대한 DEPT_TITLE이 기술연구팀으로 바뀜

ROLLBACK;
-- 위에서 커밋을 안해서 총무부로 다시 돌아와 있음
...
-- 만약에 WHERE 조건 설정하지 않으면 모든 행이 다 삭제됨
