...
-- 컬럼수를 기준으로 JOIN
--> 왼편에 작성된 테이블의 모든 행이 결과에 포함되어야 한다. (NULL이어도 나옴)
--  (JOIN이 안되는 행도 결과 포함)

-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE LEFT /* OUTER */ JOIN DEPARTMENT -- LEFT JOIN 기준으로 왼쪽
ON(DEPT_CODE = DEPT_ID); -- 23행(하동운, 이오리 포함)

-- 오라클 구문
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+); -- 반대쪽 컬럼명에 + 작성해야함
...
-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE RIGHT JOIN DEPARTMENT -- DEPARTMENT 기준으로 정렬
...
-- 모든 경우의 수를 구한 것
--> JOIN 구문을 잘못 작성하는 경우 CROSS JOIN의 결과가 조회됨 (원해서 쓰는 경우는 거의 없음)


SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
CROSS JOIN DEPARTMENT; -- 207행 == (EMLPLOYEE)23 * (DEPARTMENT)9 = 207
...
FROM EMPLOYEE
JOIN SAL_GRADE ON(SALARY BETWEEN MIN_SAL AND MAX_SAL);
-- SAL_GRADE 나 EMPLOYEE 넣어서 어디서 가져온 SAL_LEVEL인지 알려줘야함
...
-- ORA-00918: 열의 정의가 애매합니다
-- INNER JOIN 된 거임 MANAGER가 NULL인 애들은 안 나옴
...
-- DEPARTMENT 테이블에서는 DEPT_ID
-- EMPLOYEE 테이블에서는 DEPT_CODE
-- 같은 컬럼명을 가진 애가 없음
-- 결과값 CROSS JOIN (교차 조인) 값이 나옴
...
FROM EMPLOYEE
NATURAL JOIN JOB
-- NATURAL JOIN 사용하려면 JOIN 하려는 테이블과 붙여서 써야함
-- 떨어져있으면 CROSS JOIN 됨
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
--JOIN JOB USING(JOB_CODE)
...
AND E.JOB_CODE  = J.JOB_CODE -- 컬럼명 같을 때
AND JOB_NAME = '대리'
AND LOCAL_NAME LIKE 'ASIA%';
