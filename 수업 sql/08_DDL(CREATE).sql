...
CREATE TABLE USER_TEST(
		...
		PNO CHAR(14) CONSTRAINT UK_PNO UNIQUE(PNO), CONSTRAINT NN_PNO NOT NULL(PNO),
		GENDER VARCHAR2(10) CONSTRAINT CK_GENDER CHECK( GENDER IN ('남', '여') ),
		PHONE VARCHAR2(30),
		ADDRESS VARCHAR2(100),
		STATUS VARCHAR2(10) CONSTRAINT CK_STATUS CHECK (STATUS IN ('Y', 'N')), CONSTRAINT NN_STATUS NOT NULL (STATUS)
...
-- 컬럼명, 데이터 타입, 값이 복사되고, 제약조건은 NOT NULL만 복사 됨
...
--> 참조 테이블의 PRIMARY KEY(기본키)를 FOREIGN KEY로 지정하는 경우 참조컬럼명 생략 가능
...
-- * 수업시간에 활용하던 테이블에는 FOREIGN KEY 제약조건이 없는 상태이므로 추가
