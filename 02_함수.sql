...
SELECT INSTR('AABAACAABBAA', 'B', 5) FROM DUAL; -- 9 5,6,7,8,9
...
SELECT TRIM('      H E L L O         ') FROM DUAL; -- H E L L O 양쪽 공백 제거
-- 문자 사이사이 공백 제거하는 거 아님
...
-- 2024-02-29 11:20:41.377 +0900 (UTC(협정 세계시)에서 +9시간 나타냄, 한국 표준시 시간대 +0900)
...
SELECT SYSDATE FROM DUAL; -- 2024-02-29 12:21:37.000

-- 2024/02/29 12:21:37 목요일
...
SELECT TO_DATE(20240229) FROM DUAL; -- DATE 타입으로 변환
...
SELECT EMP_NAME, TO_CHAR (TO_DATE(SUBSTR (EMP_NO, 1, INSTR(EMP_NO, '-') -1), 'RRMMDD'),
'YYYY"년" MM"월" DD"일"') AS 생년월일 -- 마지막 YYYY는 RRRR로 써도 상관 없음
FROM EMPLOYEE;
...
SELECT TO_NUMBER('1,000,000', '9999999') + 5000000 FROM DUAL;
-- 6,000,000 이 모양대로 출력됨
...
-- 급여 최소값, 가장 빠른 입사일, 알파벳순서가 가장 빠른 이메일 조회
SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE;

-- 급여 최대값, 가장 늦은 입사일, 알파벳순서가 가장 마지막인 이메일 조회
...
-- COUNT(컬럼명)에 의해서 NULL을 제외한 실제 값이 기록된 행의 개수만 조회
