 ...
 *
 * - RESULT SET은 0개 이상의 행을 포함할 수 있다. (데이터 없어도 에러나지 않음)
...
-- WHERE EMP_NAME LIKE '전__'; : 이름 세글자인 사람만 포함됨
...
-- WHERE PHONE LIKE '010________'; : 010 뒤에가 8글자인 것만 포함됨
...
OR DEPT_CODE IS NULL; -- 14명 부서 코드가 없는 2명 (NULL 처리 연산자)
...
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY; -- ASC가 기본값 SALARY ASC; 이렇게 적는 거
...
ORDER BY 3 DESC; -- SELECT 절에 명시해둔 순서 SALARY 3번째에 있음

-- 입사일 순서대로 이름, 입사일 조회 (별칭 사용)
SELECT EMP_NAME 이름, HIRE_DATE 입사일
FROM EMPLOYEE
ORDER BY 입사일; -- 별칭 사용하여 정렬
